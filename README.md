...
> *"The mind is for having ideias, not holding them."*
>
...
Ela é uma espécie de "área de trabalho" para o meu cérebro: um espaço onde meus pensamentos, representados pelas notas, se tornam tangíveis e podem ser comparados, contrastados, combinados, agrupados, sequenciados, e mais qualquer coisa para qual o meio físico seja mais funcional que o mental.
    ...
    │   ╰─────────────────────╯
    │   ╭─────────────────────────────────────╮
    ├───┤ 4-Notas de estruturação de conteúdo │
    │   ╰─────────────────────────────────────╯
    │   ╭──────────╮
    ╰───┤ Projetos │
        ╰─┬────────╯
...
## Notas efêmeras (_fleeting notes_)

* Vão para a pasta 1-CAIXA DE ENTRADA para ser processadas.
* Capturam coisas que me chamaram a atenção, ideias, _insights_, etc.
    * É necessário registrar POR QUE a nota efêmera é relevante para meu raciocínio, meus interesses, minha pesquisa. Se não registrar, talvez não lembre quando for processá-la. Além disso, ajuda mais na frente a elaborar NOTAS DE AGRUPAMENTO.
* São feitas rapidamente e sem interromper o que estou fazendo – são meros LEMBRETES.
...
### Passo-a-passo para criar uma nova nota efêmera
    ...
    │     coisas que me chamaram a atenção, ideias, _insights_, etc.
    │     Pode não haver nenhuma, pode haver várias de cada.
    │
    ╰─────────
...
Por isso, vou representá-la com a sigla PQÉR.
    ...
    │ • PQÉR:   Começo muitas coisas mas termino poucas,
    │           e esse livro bem-avaliado diz que tem um método
    │           para mudar isso.
    │
...
São adicionados ao registro de uma referência no gerenciador de referências (p. ex. Zotero).

Esses destaques ficarão salvos em um campo na entrada da referência no gerenciador de referência (p. ex.  Zotero), não na Zettelkasten;
...
Ficam salvas na pasta 2-NOTAS DE REFERÊNCIA.

Estas notas devem conter apontamentos breves, e podem ser armazenadas tanto na Zettelkasten quanto no gerenciador de referências.
...
* Obrigatórios
    * Identificador
    * Título
    * Um e somente um conteúdo: uma ideia ou um pensamento ou um comentário
    * PQÉR
* Opcionais
...
* Ligação horizontal significa que a nota à direita continua a linha de raciocínio da nota à esquerda.
* Ligação vertical significa não continuar, e sim bifurcar a linha de raciocínio tomando como ponto de partida um único elemento do pensamento da nota acima e à esquerda, iniciando, portanto, uma nova linha de raciocínio a partir da nota abaixo e à direita.

O fato de duas notas estarem ligadas não implica sequência nem hierarquia.
Para dispor notas em alguma ordem – p. ex. sequencial ou hierárquica –, crio uma NOTA DE ESTRUTURAÇÃO DE CONTEÚDO.
...
* Um título declarativo também ajuda a pesquisar e descobrir se já capturei uma ideia.


#### Um e somente um conteúdo: uma ideia ou um pensamento ou um comentário

Sigo o Princípio da Atomicidade: cada nota permanente conterá uma – e apenas uma – ideia/pensamento/comentário que me ocorreu.
...
Quando a Zettelkasten tiver alcançado uma certa "massa crítica", no mais das vezes uma nota nova será ligada a uma nota existente. Cadeias de notas, sejam elas linhas de raciocínio ou ramificações, constituem _discussões_ minhas comigo mesmo.

Uma nota ficar isolada dever ser a exceção, não a regra!
...
Novos _insights_ e novas leituras ensejam novas notas permanentes, que enriquecem as discussões.
...
### Passo-a-passo para criar uma nova nota permanente
    ...
    │   cujo atalho é CTRL+O,
    │   que permite pesquisar notas pelo título.
    │
    │   • Se já houver uma nota com a mesma ideia, não preciso prosseguir.
    │
    │   • Se a ideia for totalmente nova,
    ...
    │   • Se a ideia for a continuação ou ramificação de uma existente,
    │     crio o identificador de acordo com a nota relacionada.
    │
    │   • Clico em "Inserir modelo" (Insert template)
    ...
    │   e o(s) termo(s) NÃO devem ser de arquivista,
    │   e sim de escritor.
...
* Indiretamente, ligada que deve estar a uma nota existente que serve como ponto de entrada para uma linha de raciocínio;
* Diretamente, servindo como ponto de entrada para uma linha de raciocínio.
...
## Notas de estruturação de conteúdo

Ficam salvas na pasta 4-NOTAS DE ESTRUTURAÇÃO DE CONTEÚDO.

Notas de estruturação de conteúdo servem como esboços de uma estrutura de tópicos sobre um dado assunto.
...
### Passo-a-passo para criar uma nova nota de estruturação de conteúdo

    ╭──ANTES──
    │
    │   • Clico com o botão direito na pasta "4-Notas de estruturação de conteúdo"
    │     e seleciono "Nova nota" (New note).
    │
    │   • Clico em "Inserir modelo" (Insert template)
    │     e seleciono "4-nota de estruturação de conteúdo".
    │
